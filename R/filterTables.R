#' @title Filter data for normalization set construction
#' @description Filter all data sets by specified criteria on fold changes and metadata columns
#' @details ...
#' @seealso ...
#' @family ...
#' @references ...
...
filterTables <- function(data, normReqs){
  ...
  message("\t1. Filtering by other columns:")
  ...
  fcListFiltered <- sapply(grNames, function(n){message("Filtering by annotation columns ", paste(cols, collapse=" "), " in treatment group: ", n)
