#' Plot QC plot to visualize curve fit of normalization curves to fold change medians.
#' @param modelList ...
#' @param xMat ...
#' @param fcMat ...
#' @param r2Vec ...
...
#' @param plotTheme ...
#' @param pathPlot ...
plotNormCurves <- function(modelList, xMat, fcMat, r2Vec, nNormP, plotTheme, pathPlot){
  ...
  p <- p + xlab(paste('Temperature [\U00B0', 'C]', sep='')) + ylab("Median fold change")
  p <- p + geom_line(data=plotDF_curves, aes(x=Temperature, y=FoldChange, colour=condition), size=1 )
  p <- p + geom_point(data=plotDF_points, aes(x=Temperature, y=FoldChange, colour=condition), na.rm=T, size=4)
  p <- p + geom_text(data=plotDF_anno, aes(x=xPos, y=yPos, label=lab))
