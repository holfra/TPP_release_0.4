...
#' @param resultPath location where to store dose-response curve plots and result table.
#' @param ggplotTheme ggplot theme for dose response curve plots.
#' @param doPlot boolan value indicating whether dose response curves should 
...
#' @export
#' @details 
#' \code{data} is an ExpressionSet object created by \code{\link{tppccrImport}}.
#' It contains the isobaric labels and administered drug concentrations in the 
#' \code{phenoData} and user-defined protein properties in the \code{featureData}.
...
tppccrCurveFit <- function(data, resultPath, ggplotTheme=tppDefaultTheme(), 
                           doPlot=TRUE, fcCutoff=1.5, r2Cutoff=0.8, slopeBounds=c(1,50)){
  ...
  for(i in 1:nrow(data)) {
    ...
    if(length(not_na)>0) {
      ...
      ## Retrieve dose (independent variable) and response (dependent variable) values:
      ...
      if(class(fit) != "try-error") {
        ...
        if (doPlot){
          ...
          yvals <- eval(parse(text=fctSigmoidCCR()), envir=list(x=xvals, infl=pec50, hill=hill))
          ylab <- "normalized apparent stability\n(relative to highest ligand concentration)"
          if(cpd_effect=="destabilized") {
            ylab <- "normalized apparent stability\n(relative to vehicle)"
          }
          #mainStr <- paste("ID:", ipi, "\npEC50:", pec50_final, ", Hill:", hill, ", R2:", r2, "\n", "Experiment:", expName)
          p <- ggplot()
          p <- p + scale_y_continuous(limits=ylim) + scale_x_continuous(limits=xlim)
          p <- p + ggtitle(ipi)
          p <- p + xlab("cpd. conc. (log M)") + ylab(ylab)
          p <- p + geom_point(data=data.frame(dose, response), mapping=aes(x=dose, y=response), na.rm=T, size=4)
          p <- p + geom_line(data=data.frame(xvals, yvals), mapping=aes(x=xvals, y=yvals), colour="red")
          
          tableDF <- data.frame(condition = expName,
                                pEC50   = signif(pec50_final,2),
                                slope   = signif(hill,2),
                                R2      = signif(r2,2))
          p <- addTableToPlot(plotObj=p, tableDF=tableDF, meltVar="condition", clrs="red")
          
          
          file_name  <- file.path(plotDir, paste("drCurve_",gsub("([^[:alnum:]])", "_", ipi),".pdf", sep=""))
          ggsave(filename=file.path(resultPath, file_name), plot=p, width=20, height=25, units="cm")
        }
      }
    }
    ...
    fitResults <- rbind(fitResults, data.frame(Protein_ID=ipi, pEC50=pec50_final,
                                               slope=hill, R_sq=r2, passed_filter=passed_both,
                                               pEC50_outside_conc_range=flagStr,
                                               Plot=file_name, stringsAsFactors=F))      
  }
  ...
  dfFCs   <- data.frame(Protein_ID=protIDs, fcMat, stringsAsFactors=F)
  ...
  dfFeatureDat2 <- data.frame(Protein_ID=featureNames(data), dfFeatureDat, stringsAsFactors=F)
