...
#' @description Summarizes the output of a TPP-TR experiment and performs statistical
#' comparisons of conditions, if appropriate.
#' @details If a TPP-TR experiment was performed with the conditions "Vehicle" and "Treatment",
...
tpptrResultTable <- function(data, binWidth=300){
  ...
  for (en in expNames){
    ...
    colnamesCurvePars  <- meltCurveParamNames(returnParNames=T, returnPerformanceInfo=F)
    colnamesModelInfo  <- meltCurveParamNames(returnParNames=F, returnPerformanceInfo=T)
    colnamesOthers     <- setdiff(colnames(dfAnnotDataTmp), c(colnamesCurvePars, colnamesModelInfo, "plot"))
    ...
    if (flagIsNormalized) colnames(dfFoldchangesTmp) <- paste("norm", colnames(dfFoldchangesTmp), sep="_")
    ...
    resultColsMeltcurvePars <- data.frame(Protein_ID=protIDsTmp, dfMeltCurveParsTmp, stringsAsFactors=F)
    resultColsModelInfo     <- data.frame(Protein_ID=protIDsTmp, dfModelInfoTmp, stringsAsFactors=F)
    resultColsAnnot         <- data.frame(Protein_ID=protIDsTmp, dfOtherAnnotationTmp, stringsAsFactors=F)
    resultColsFC            <- data.frame(Protein_ID=protIDsTmp, dfFoldchangesTmp, stringsAsFactors=F)
  ...
  if (length(condLevels)==2){
    if (all.equal(sort(as.character(condLevels)), c("Treatment", "Vehicle"))){
      ...
      if (max(table(paste(expConds, expRepls))) == 1) flagCompareMP <- TRUE
    }
  }
  ...
  if (flagCompareMP){
    for (r in repLevels){
      ...
      statsTmp <- data.frame(Protein_ID=outTable$Protein_ID, stringsAsFactors=F)
      ...
      passedTest <-resultFilterCurvePars(r2V=r2V, r2T=r2T, plV=plV) # Test: R2 > 0.8 (both columns) + Plateau < 0.3 (Vehicle)
      ...
      pVals <- computePvalues(ids=idsNew, minSlopes=minslNew, mpDiffs=mpdiffNew, binWidth=binWidthNew)

      ## Store p-values:
      pvalDF <- data.frame(Protein_ID=idsNew, stringsAsFactors=F)
    ...
    if(flagCompareRep){
      ...
      pValMin <- pmin(col1, col2, na.rm = F) # Only sucessful, if both p-values are non-missing values
      pValMax <- pmax(col1, col2, na.rm = F) # Only sucessful, if both p-values are non-missing values
